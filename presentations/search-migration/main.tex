...
\begin{frame}{Search Concepts: From Similarity to Relevance}
  ...
  \end{columns}
  \vspace{0.2cm}
  \begin{block}{Migration implication}
    There is no equivalent 0.3 BM25 cutoff. We tune matching and ranking separately, then validate the results.
  \end{block}
\end{frame}
...
\begin{frame}{Validation: Generated Queries with NLPAUG}
  ...
  \begin{block}{First: generated test cases}
    Use \href{https://github.com/makcedward/nlpaug}{\underline{NLPAUG}} to create variations of company-name queries and compare search behavior across both setups.
  \end{block}
  \begin{itemize}
    \item Exercise noisy input beyond exact company names.
    \item Compare how both search setups handle the generated queries.
  \end{itemize}
  \begin{block}{Then: historical queries and expert votes}
    Replay customer queries and ask experts to compare both top-20 result lists.
  \end{block}
\end{frame}
...
\begin{frame}{Validation: MRR Results}
  \begin{center}
    \begin{tabular}{lll}
      \toprule
      Search system & MRR (found only) & Not found \\
      \midrule
      PostgreSQL \texttt{pg\_trgm} & Awaiting value & Awaiting value \\
      Elasticsearch & Awaiting value & Awaiting value \\
      \bottomrule
    \end{tabular}
  \end{center}
  \begin{block}{Draft: evaluation details to add}
    Query count, cutoff $k$, relevance labels, country / dataset scope, and not-found counts or rates.
  \end{block}
  \small MRR averages only queries with a relevant result in the top $k$; misses are reported separately. Unlike standard MRR, misses do not contribute zero here.
  \par\smallskip
  \scriptsize Same input query set; each system's found-only subset may differ. Placeholders are not results.
\end{frame}
